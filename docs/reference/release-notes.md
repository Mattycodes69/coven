...
- **`coven --version` works on source builds (v0.0.18).** Native binaries produced by `cargo install`, `cargo build`, or extracted directly from a published npm tarball now answer `coven --version` / `coven -V` instead of returning an "unexpected argument" error. A `build.rs` stamps the binary with `git describe --tags --always --dirty` for local checkouts (e.g. `v0.0.17-2-g00a9c16-dirty`), falls back to `$GITHUB_REF_NAME` so CI-tag-triggered releases stamp the bare tag (e.g. `v0.0.18`), and falls back further to `<CARGO_PKG_VERSION> (unknown source)` for tarball builds without git. The npm wrapper (`@opencoven/cli`) still short-circuits `--version` to print its own `package.json` version, so npm users see no behavioral change.
